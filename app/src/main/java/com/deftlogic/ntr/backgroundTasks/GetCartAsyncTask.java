 ...
 */
public class GetCartAsyncTask extends AsyncTask<Void, Void, List<ItemDetail> > {
    private Context mContext;
    private CartResultsCallback cartResultsCallback;
    private List<ItemDetail> itemDetailList ;
    Cursor mCursor;
    FieldAssistDbHelper fieldAssistDbHelper;
    SQLiteDatabase db;

    public GetCartAsyncTask(CartResultsCallback cartResultsCallback,Context context) {
        this.mContext = context;
        this.cartResultsCallback = cartResultsCallback;
    }
    ...
    protected void onPostExecute(List<ItemDetail> itemDetailList) {
        ...
        {
            cartResultsCallback.onItemSuccess(itemDetailList);}
        else
        {
            cartResultsCallback.onFailure(mContext.getResources().getString(R.string.failure_to_load));
        }
        db.close();
    ...
    private List<ItemDetail> parseResult() {
        ...
        String mSelectionClause = FieldAssistContract.ItemViewDetailsEntry.COLUMN_ITEM_CART_COUNT + " > ?";
